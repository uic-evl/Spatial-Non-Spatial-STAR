...
App.setupDB = function(data, tabletop) {
    ...
    App.db = DB.initializeDB(App.dbName, App.rows);
};

App.setupTable = function(data) {
...
App.setupCharts = function(data){

    /** initialize a new bubble graph **/
    App.graph = new Graph({colorMap: App.colorMap});

    App.dataParser = new Parser({colorMap: App.colorMap});
    ...
    if(taskData.subDomains) subDomains = taskData.subDomains;
    ...
    App.graph.graphEncodingBubbleNVD3Chart(encodingData.encodings, "#encodings",
        encodingData.max, encodingData.groups, encodingData.authors, subDomains);
    ...
    App.graph.graphTaskBarNVD3Chart(taskData.tasks, "#tasks", 0, taskData.groups,
        subDomains, taskData.authors[0], taskData.count);

    // // plot the data type analysis
    App.graph.graphTypeBarNVD3Chart(taskData.dataTypes, "#dataTypes", 0,
        ["Table", "Field", "Network", "Geometry"], subDomains, taskData.authors[1], taskData.count);

    // // plot the data type analysis
    App.graph.graphEvaluationNVD3Chart(taskData.evaluation, "#evaluation", 0,
        ["Table", "Field", "Network", "Geometry"], subDomains, taskData.authors[2], taskData.count);

    // plot the data type analysis
    App.graph.graphParadigmsNVD3Chart(taskData.paradigms, "#paradigms", 0,
        ["Table", "Field", "Network", "Geometry"], subDomains, taskData.authors[3], taskData.count);

    // // plot the data type analysis
    App.graph.graphEvaluatorsNVD3Chart(taskData.evaluators, "#evaluators", 0,
        ["Table", "Field", "Network", "Geometry"], subDomains, taskData.authors[3], taskData.count);
};
...
App.getResults = function(e) {
    ...
    DB.queryPapers({and: [input, operator, values], or: advanced}, App.setupTable);
